...
## AI Provider (custom, tanpa gateway AI Viktor)

Versi ini memakai **provider AI custom (OpenAI-compatible) secara langsung** untuk
generate struktur, pertanyaan, dan PRD. Kodenya ada di `convex/aiClient.ts` dan
dipakai oleh `convex/prdActions.ts`.

Set environment variable di Convex:

```bash
npx convex env set AI_API_KEY "sk-..."         # WAJIB: API key provider
npx convex env set AI_MODEL "gpt-4o"           # opsional, default gpt-4o
npx convex env set AI_BASE_URL "https://api.openai.com/v1"  # opsional
```

- `AI_BASE_URL` bisa diarahkan ke provider OpenAI-compatible lain (OpenRouter,
  Groq, Together, Azure OpenAI, dll).
- Untuk **kembali memakai gateway AI Viktor**, cukup ganti impor di
  `convex/prdActions.ts` dari `./aiClient` menjadi `./viktorClient`.
